...
function start() {
  var local = (host === '0.0.0.0' || host === "localhost" || host === "127.0.0.1");
  ...
  function bind() {
    responder.on('message', function (req, reply) {
      queue.push({ req: req, reply: reply });
      processQueue();
    });
  }
}
...
function processQueue() {
  ...
  if (queue.length && !queue.pending) {
    var next = queue.pop();
    queue.pending = true;

    processMessage(next.req, function () {
      // FIXME decide whether `this` is okay to use
      next.reply.apply(this, arguments);
      queue.pending = false;
      processQueue();
    });
  }
...
function processMessage(req, reply) {
    ...
    var p = new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        psTree(child.pid, function (err, children) {
          var pids = [child.pid].concat(children.map(function (p) {
            return p.PID;
          }));
          spawn('kill', ['-s', 'SIGTERM'].concat(pids));
        });
